<?php
	...
	class NinjaFormAjaxForm extends FormHandler implements IFormHandler
	{
		...
		function enqueue_nj_form_script($form_id)
		{
			if(array_key_exists($form_id,$this->_formDetails))
			{
				...
				wp_localize_script('njscript', 'moninjavars', array(
					...
					'verifyField'	=>	get_mo_option('nfa_verify_field'),
					'siteURL' 		=> 	site_url(),
				));
		...
		function _add_button($fields)
		{
			...
			foreach ($fields as $key => $field)
			{
				if($field['id']==$formdata[$fieldKey]){
					...
					value="'.mo_($this->_buttonText).'" type="button"></div></div></div><div class="nf-after-field"><nf-section><div class="nf-input-limit">
		...
		function processOTPEntered($data,$formdata)
		{
			...
			$this->validateChallenge($data['fields'][$verify_field]['value']);
		...
		{
            $sessionKey = strcasecmp($this->_otpType,$this->_typePhoneTag)==0 ? $this->_formPhoneVer : $this->_formEmailVer;
		...
		function handleFormOptions()
		{
			...
			update_mo_option('ninja_form_otp_enabled',$this->_formDetails);
		}
