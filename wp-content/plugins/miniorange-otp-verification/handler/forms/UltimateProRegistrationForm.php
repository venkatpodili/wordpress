	...
	class UltimateProRegistrationForm extends FormHandler implements IFormHandler
	{
		...
		function _handle_ulti_form($data)
		{
			...
			if(strcasecmp($this->_otpType,$this->_typePhoneTag)==0)
			{
				...
				$this->sendChallenge('testuser',null,null,$data['user_phone'],"phone");
			}
			else
			{
				$_SESSION[$this->_formEmailVer] = trim($data['user_email']);
				$this->sendChallenge('testuser',$data['user_email'],null,null,"email");
			}
		}
		...
		{
			$this->validateChallenge(NULL,$otpToken);
