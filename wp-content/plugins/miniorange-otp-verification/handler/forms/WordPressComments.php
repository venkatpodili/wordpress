	...
	class WordPressComments extends FormHandler implements IFormHandler
	{
		...
		function _startOTPVerificationProcess($getdata)
		{
				...
				&& !MoUtility::isBlank($getdata['user_email']))
			{
				...
				$this->sendChallenge('test',$getdata['user_email'],null,$getdata['user_email'],"email");
			}
				...
				&& !MoUtility::isBlank($getdata['user_phone']))
			{
				...
				$this->sendChallenge('test','',null, trim($getdata['user_phone']),"phone");
			}
			else
			{
		...
		function verify_comment_meta_data( $commentdata ) 
		{
			...
			MoUtility::checkSession();
		  	if ( ! isset( $_POST['phone'] ) && strcasecmp($this->_otpType,$this->_typePhoneTag)===0)
		  		wp_die( mo_( 'Error: You did not add a phone number. Hit the Back button on your Web browser and resubmit your comment with a phone number.'  ) );
							  
		  	if ( ! isset( $_POST['verifyotp'] ) )
		  		wp_die( mo_( 'Error: You did not add a Verification Code. Hit the Back button on your Web browser and resubmit your comment with a verification code.'  ) );
			...
			$this->validateChallenge(NULL,$_POST['verifyotp']);
