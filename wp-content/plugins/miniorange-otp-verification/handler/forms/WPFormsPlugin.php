<?php
	...
	class WPFormsPlugin extends FormHandler implements IFormHandler
	{
		...
		{
			return get_mo_option('wpform_enable') ? true : false;
		}
		...
		public function showVerificationButton($field,$formData)
		{
			...
			if($this->_otpType===$this->_typePhoneTag && strcasecmp($field['label'],$formData['phone_show'])===0) {
				echo $this->getButtonAndScriptCode('phone',$formData,$formId);
			}
			elseif($this->_otpType===$this->_typeEmailTag && strcasecmp($field['label'],$formData['email_show'])===0) {
		...
		function processOTPEntered($formdata,$errors,$id)
		{
			...
			$this->validateChallenge(NULL,$_POST['wpforms']['fields'][$verify_field]);
		...
		function handleFormOptions()
	    {
			...
			update_mo_option('wpform_forms', $this->_formDetails);
		}

		function parseFormDetails()
		{
			if(!array_key_exists('wpform_form',$_POST)) return array();
