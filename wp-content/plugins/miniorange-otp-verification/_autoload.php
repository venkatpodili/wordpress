<?php
	...
	define('MO_FAIL_MODE', FALSE);

	    includeFile(MOV_DIR . 'traits');
	...
	includeFile(MOV_DIR . 'handler',$handlerList);
	...
	function includeFile($folder,$handlerList = null)
	{
		foreach (scandir($folder) as $filename)
		{
		    ...
		    if (is_file($path) && strpos($filename, '.php') !== false) {
				require_once $path;
				if(!is_null($handlerList) && strpos($folder,MOV_DIR)!==false) {
				    initializeForms($folder,$filename,$handlerList);
                }
			} elseif(is_dir($path) && isValidFile($filename)) {
				includeFile($folder . '/' . $filename,$handlerList);
			}
		}
	}

    
	function initializeForms($folder,$filename,$handlerList)
    {
