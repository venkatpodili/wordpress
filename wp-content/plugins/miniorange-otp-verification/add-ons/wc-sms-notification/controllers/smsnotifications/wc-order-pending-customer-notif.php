<?php
	...
	{
		$notification_settings->getWcOrderPendingNotif()->setIsEnabled(array_key_exists($enableDisableTag, $_POST) ? TRUE : FALSE);
		if(array_key_exists($recipientTag, $_POST))
			$notification_settings->getWcOrderPendingNotif()->setRecipient(serialize(explode(";",$_POST[$recipientTag])));
		if(array_key_exists($textareaTag, $_POST))
			$notification_settings->getWcOrderPendingNotif()->setSmsBody(
