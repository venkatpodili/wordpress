	...
	class WooCommerceNotifications extends BaseAddonHandler
	{
		...
		function _send_custom_order_msg($POST)
		{
			if(!array_key_exists('numbers',$POST) || MoUtility::isBlank($POST['numbers']))
				MoUtility::_create_json_response(MoWcAddOnMessages::showMessage("INVALID_PHONE"),MoConstants::ERROR_JSON_TYPE);
			else
			{
				foreach (explode(";",$POST['numbers']) as $number) {
					if(MoUtility::send_phone_notif($number,$POST['msg']))
						wp_send_json(MoUtility::_create_json_response(
							MoWcAddOnMessages::showMessage("SMS_SENT_SUCCESS"),MoConstants::SUCCESS_JSON_TYPE));
					else
						wp_send_json(MoUtility::_create_json_response(
							MoWcAddOnMessages::showMessage("ERROR_SENDING_SMS"),MoConstants::ERROR_JSON_TYPE));
				}
			}
		}
